...
async function main() {
  ...
  try {
    ...
    // サインアップモーダル等のポップアップを自動で閉じる
    await page.evaluate(() => {
      // 右下のサインアップモーダルを削除
      document.querySelectorAll('[class*="AuthModal"], [class*="signup"], [class*="signin"]').forEach(el => el.remove());
      // ×ボタンがあればクリック
      document.querySelectorAll('button[aria-label="Close"], button[aria-label="閉じる"]').forEach((el: any) => el.click());
    }).catch(() => {});
    // ESCキーも念のため
